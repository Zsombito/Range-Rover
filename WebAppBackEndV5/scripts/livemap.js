...
function writeobj(x_pos,y_pos, color,type,rad,c){
    rad = rad_obj[type]
    c.fillStyle = color 
    if(type == "fan"){
...
function deleteobj(pos,type,c){
    rad = rad_obj[type]
    c.clearRect(pos.x_pos/3,pos.y_pos/3,65,65)
    old_pos = curr_pos
}
...
async function writerover(pos,c){
    ...
    c.drawImage(img,pos.x_pos/3,pos.y_pos/3)
...
async function rcvrovdata(url,c) {
    ...
    let latencyclient = performance.now() - starttime
    console.log(latencyclient)
    console.log(data)
...
async function rcvobjdata(url,c) {
    ...
    let data = await response.json();
    if(data.valid){
        await writeobj(data.x_pos,data.y_pos,data.colour,data.obstacle,data.radius, c)
    }
...
function mousetip(e) {
    ...
    //console.log(pos)
    tooltip.innerHTML = "("+Math.round(pos.xcord)+","+Math.round(pos.ycord)+")"
