...
server.post('/drive/directed', function(req,res) {
    ...
    try {
        var htmlc = fileSystem.readFileSync('/home/ubuntu/WebAppBackEnd3/html/drive_directed.html', 'utf8');
      } catch (err) {
        console.error(err);
      }
...
server.get('/view/objpos',function(req,res){
    let q = "SELECT x_pos,y_pos,colour,obstacle FROM Obstacles WHERE time = (SELECT MAX(time) FROM Obstacles);"
    var r
    res.writeHead(200, {'Content-Type':'application/json'})
    obj = {x_pos:i,y_pos:i}
    i++
